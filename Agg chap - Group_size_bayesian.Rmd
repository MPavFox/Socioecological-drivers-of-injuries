...
Does the number of males in the group predict male male within group escalated competition? It is expected that males will be more involved in between group encounters than within group encounters, so there should not be an effect on physical aggression.
```{r}
...
Ruling out inter-sexual competition over food as driver of injuries
```{r}
...
#Female to Male aggression events are very rare, thus not possible to explore
table(males_agg$from_FWS)#9/3154 bimonthly intervals
...
post2 = posterior_samples(male_fem_all)
post3 = posterior_samples(male_agg)

posts = bind_cols(post1 %>% select(b_scaled_fem),
                  post2 %>% select(b_scaled_group),
...
posts %<>% mutate(predictor = ifelse(predictor == "scaled_fem","Female_count", predictor),
                  predictor = ifelse(predictor =="scaled_group","Group_size", predictor),
                  predictor = ifelse(predictor == "scaled_male","Male_count", predictor),
                  predictor = ordered(predictor, levels = c("Male_count","Group_size","Female_count")))

Fig2 = posts %>% 
  ggplot(aes(x = value, y = predictor, fill = predictor)) +
  stat_halfeye(slab_alpha = 0.5, .width = c(0.66,0.89)) +
  scale_fill_manual(values = c("goldenrod2","cyan4","cyan4")) +
  geom_vline(xintercept = 0, linetype =  "dashed", color = "gray28", linewidth = 1)+
  scale_color_manual(values = c("goldenrod2","cyan4","cyan4")) + scale_x_continuous(name = c("Estimate")) +
  scale_y_discrete(name = c("Density"), expand = c(0, 0.1), labels = c( "Male count","Group size", "Female count"))+ theme_bw() +
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(), axis.line = element_line(colour = "black"),
        text = element_text(size = 18),
...
FigS5: Effect of sex ratio on female-female non-contact
```{r}

#Load model
fem_SR_NCagg <- readRDS(paste0(directory,"Bmrs models/fem_SR_NCagg.rds"))

#extract posterior
post = posterior_samples(fem_SR_NCagg)

FigS5 = post %>% 
  ggplot(aes(x = b_scaled_SR)) +
  stat_halfeye(fill = "cyan4",slab_alpha = 0.5, .width = c(0.66,0.89)) +
  geom_vline(xintercept = 0, linetype =  "dashed", color = "gray28", size = 1)+
  scale_fill_manual(values = c("cyan4")) +
  scale_color_manual(values = c("cyan4")) + scale_x_continuous(name = c("Estimate")) +
  scale_y_discrete(name = c("Density"), expand = c(0, 0.01), labels = c("Sex Ratio")) + theme_bw() +
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(), axis.line = element_line(colour = "black"),
        text = element_text(size = 18),
        axis.text.y=element_text(size=15)) 
